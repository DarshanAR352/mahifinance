...
function drawTableRow(doc, y, description, unitPrice, quantity, total, isHeader) {
    ...
    this.on('generateInvoice', async (req) => {
        ...
        const file = {
          ...
          debugger
          const fileID = result.query.INSERT.entries[0].ID;
          debugger
  ...
  async function fetchCsrfToken() {
    ...
    this.on('generateSO', async (req) => { 
        ...
        const SoID = Math.floor(10000000 + Math.random() * 90000000).toString();
      ...
      await INSERT.into(PaymentSettings).entries({
        soID : SoID
      });
      ...
      this.on('sendForRelease', async (req) => { 
        ...
        const workflowData =
        {
            ...
            "context": {
                ...
                "link": "http://example.com/customer/CUST12345",
